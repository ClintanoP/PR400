...
class Canvas:
    ...
    def render_hands(self, event):
        global hands_data, asl_model, predicted_gesture, predicted_confidence, batch_size, hand_colour
...
def process_and_predict(self, hands_data, asl_model, labels_list, output_queue):
    ...
    hands_data['Left'] = hands_data['Left'][-45:]
    hands_data['Right'] = hands_data['Right'][-45:]
    hands_data['Count'] = 45

    updated_data = {
        'hands_data': hands_data,  # Assuming hands_data is updated within this function
        'predicted_gesture': predicted_gesture,
        'predicted_confidence': predicted_confidence,
    }

    # Put the bundled data into the queue
...
def main():
    ...
    with connection.open():
        ...
        while running:
            ...
            try:
                while not hands_data_queue.empty():
                    updated_data = hands_data_queue.get_nowait()
            
                    # Update the global variables with the processed data
                    hands_data.update(updated_data['hands_data'])  # Update hands_data as before
                    
                    # Now also update the gesture and confidence
                    global predicted_gesture, predicted_confidence  # Declare as global if updating globals inside a function
                    predicted_gesture = updated_data['predicted_gesture']
                    predicted_confidence = updated_data['predicted_confidence']
            except Queue.Empty:
                pass
